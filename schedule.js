...
const UI = require('solid-app-set')
...
//    var forms_uri = 'https://linkeddata.github.io/app-schedule/forms.ttl'; // CORS blocks
  ...
  var complainIfBad = function (ok, message) {
    if (!ok) {
      div.appendChild(UI.panes.utils.errorMessageBlock(dom, message, 'pink'))
    }
  }
  ...
  var me_uri = UI.preferences.get('me')
  var me = me_uri ? kb.sym(me_uri) : null
  UI.panes.utils.checkUser(detailsDoc, setUser)
  ...
  var newInstanceButton = function () {
    return UI.panes.utils.newAppInstance(dom, {noun: 'poll'},
...
//            { local: 'mashlib.js', contentType: 'application/javascript'} , //  @@ centrialize after testing?
    ...
    for (f = 0; f < toBeCopied.length; f++) {
      ...
      UI.panes.utils.checkUser(xhr.resource, function (webid) {
				    me = kb.sym(webid)
  ...
  var showAppropriateDisplay = function () {
    ...
    if (!me) {
      ...
      if (!UI.sparql.editable(detailsDoc.uri, kb) ||
  ...
  var showBootstrap = function showBootstrap () {
    ...
    var na = div.appendChild(UI.panes.utils.newAppInstance(
  ...
  var showForms = function () {
    ...
    if (wizard) {
      ...
      for (var f = 0; f < forms.length; f++) {
        ...
        UI.panes.utils.appendForm(document, slide, {}, subject, forms[f], detailsDoc, complainIfBad)
        slides.push(slide)
      }
      ...
      UI.panes.utils.appendForm(document, table, {}, subject, form1, detailsDoc, complainIfBad)
      UI.panes.utils.appendForm(document, table, {}, subject, form2, detailsDoc, complainIfBad)
      UI.panes.utils.appendForm(document, table, {}, subject, form3, detailsDoc, complainIfBad)
      naviCenter.appendChild(doneButton) // could also check data shape
    }
    ...
    doneButton.addEventListener('click', function (e) {
      if (kb.any(subject, SCHED('ready'))) { // already done
        ...
        naviRight.appendChild(emailButton)
        UI.sparql.update([], insertables, function (uri, success, error_body) {
          if (!success) {
  ...
  var setTimesOfDay = function () {
    ...
    if (slots.length === 0) {
      for (i = 0; i < 2; i++) {
        slot = UI.panes.utils.newThing(detailsDoc)
        ...
        for (j = 0; j < possibleDays.length; j++) {
          ...
          cell = UI.panes.utils.newThing(detailsDoc)
    ...
    var displayTheMatrix = function () {
      ...
      refreshButton.addEventListener('click', function (e) {
        ...
        UI.sf.nowOrWhenFetched(subject_uri.split('#')[0], undefined, function (ok, body) {
          if (!ok) {
    ...
    var id = UI.panes.utils.newThing(doc).uri
    if (myResponse === null) {
    ...
    if (insertables.length) {
      kb.updater.update([], insertables, function (uri, success, error_body) {
        if (!success) {
          complainIfBad(success, error_body)
        } else {
          displayTheMatrix()
  ...
  var showResults = function () {
    ...
    var displayTheMatrix = function () {
      ...
      refreshButton.addEventListener('click', function (e) {
        refreshButton.disabled = true
        kb.fetcher.nowOrWhenFetched(subject_uri.split('#')[0], undefined, function (ok, body) {
          if (!ok) {
            console.log('Cant refresh matrix' + body)
          } else {
            matrix.refresh()
  ...
  var logInOutButton = null
    /*
    var logInOutButton = UI.panes.utils.loginStatusBox(dom, setUser);
